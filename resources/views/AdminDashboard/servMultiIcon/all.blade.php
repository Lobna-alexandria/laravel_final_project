...
@section('page_title','All all Social Icons')
...
<div class="card">
    <div class="card-header">
      ...
      <a href="{{route('ServeMultiIcon.create')}}" class="btn bg-olive mx-5">Add Services Icon</a>

      @else
      <button disabled class="btn btn-secondary mx-5">Add Services Icon</button>
    ...
    <div class="card-body">
      <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4"><div class="row">
        ...
        <tr role="row">
            ...
            <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Link Name</th>
        ...
        <tbody>
         <tr role="row" class="odd">
          @foreach ($allSIcons as $data)
                    
          <td tabindex="0" class="sorting_1">{{$data->id}}</td>
          <td>{{$data->linkname}}</td>
          <td><img src="{{asset('uploads/ServMultiIcons/'.$data->image)}}" width="100" height="100" alt=""> </td>
          ...
          <td>
             ...
             @if (Auth::user()->hasPermission('users-update'))
                 ...
                 <a href="{{route('ServeMultiIcon.edit',['ServeMultiIcon' => $data->id ])}}" class="btn bg-olive">Edit  </a>
            ...
            <form action="{{route('ServeMultiIcon.destroy',['ServeMultiIcon'=>$data->id])}}" method="POST">
              @csrf
              @method('DELETE')
