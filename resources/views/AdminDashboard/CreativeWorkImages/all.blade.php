...
@section('page_title','All Creative Works Images')
...
<div class="card">
    <div class="card-header">
      ...
      <a href="{{route('CreativeWorkImg.create')}}" class="btn bg-olive mx-5">Add Images</a>

      @else
      <button disabled class="btn btn-secondary mx-5">Add Images</button>
    ...
    <div class="card-body">
      <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4"><div class="row">
        ...
        <tr role="row">
            ...
            <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">crwork_id</th>
        ...
        <tbody>
         <tr role="row" class="odd">
          ...
          <td tabindex="0" class="sorting_1">{{$data->id}}</td>
           <td>{{$data->desc}}</td>
          <td><img src="{{asset('uploads/crimages/'.$data->image)}}" alt="" width="100" height="100" class="img_fluid"></td>
         {{-- الاسم من الجدول الاخر --}}
          <td>{{$data->CreativeWorks->name}}</td>
          ...
          <td>
             ...
             @if (Auth::user()->hasPermission('users-update'))
                 ...
                 <a href="{{route('CreativeWorkImg.edit',['CreativeWorkImg' => $data->id ])}}" class="btn bg-olive">Edit  </a>
            ...
            <form action="{{route('CreativeWorkImg.destroy',['CreativeWorkImg'=>$data->id])}}" method="POST">
              @csrf
              @method('DELETE')
